...
def menu(mpp):
    choice = input(f"\n{colors.OKBLUE}>{colors.ENDC} ").lower()
...
def init_db(mpp):
    cur.execute(
        "CREATE TABLE passwords (service_name text UNIQUE, service_url text UNIQUE, identifier text, kdf_salt blob, ciphertext blob, nonce blob, auth_tag blob, created_at date);"
    )
...
def create_pass(mpp):
    service_name = input("enter the name of the service: ").lower()
    service_url = input("enter the URL of the service: ").lower()
    ...
    if ent_or_gen == "y":
        ...
        if password == c_password:
            ...
            if password == c_password:
                ...
                print(f"{colors.FAIL}two failed attempts{colors.ENDC}")
                quit()
    ...
    try:
        ...
        print(
            f"{colors.WARNING}a record for the entered service already exists, use the help command for help{colors.ENDC}"
        )
...
def read_pass(mpp):
    ...
    try:
        cur.execute(
            "SELECT kdf_salt, ciphertext, nonce, auth_tag FROM passwords WHERE service_name=(?)",
            (service_name,),
        )
    except sqlite3.OperationalError:
        print(
            f"{colors.WARNING}database is not initialized yet, use the init command to do so{colors.ENDC}"
        )
...
def update_pass(mpp):
    ...
    if acc is not None:
        ...
        cur.execute(
            "UPDATE passwords SET kdf_salt=? ,ciphertext=?, nonce=?, auth_tag=?, created_at=? WHERE service_name=? ",
            (kdf_salt, ciphertext, nonce, auth_tag, datetime.now(), service_name),
        )
...
def update_master_pass():
    ...
    confirm_new_master_pass = getpass("confirm new master password: ").encode("utf-8")
    if new_master_pass == confirm_new_master_pass:
...
def show_help(mpp):
    print(
        f"{colors.OKBLUE}init\t[in]{colors.ENDC}\t\t{colors.OKGREEN}initalizes a new database, run this if this is the first time running this program{colors.ENDC}\n{colors.OKBLUE}add\t[a]{colors.ENDC}\t\t{colors.OKGREEN}adds a new password{colors.ENDC}\n{colors.OKBLUE}read\t[r]{colors.ENDC}\t\t{colors.OKGREEN}reads a password{colors.ENDC}\n{colors.OKBLUE}update\t[u]{colors.ENDC}\t\t{colors.OKGREEN}updates a password{colors.ENDC}\n{colors.OKBLUE}info\t[i]{colors.ENDC}\t\t{colors.OKGREEN}returns info associated with the password{colors.ENDC}\n{colors.OKBLUE}master\t[m]{colors.ENDC}\t\t{colors.OKGREEN}changes the master password{colors.ENDC}\n{colors.OKBLUE}help\t[h]{colors.ENDC}\t\t{colors.OKGREEN}shows this help message{colors.ENDC}"
    )
